...
def death_gui():
    ...
    for k in death_info['Top 10 Worst(best) players']:
        ...
        y+=27
        tk.Label(root, text=k[0]+' '+"{:,}".format(k[1])+' Fame',font=("Helvetica", 10, "bold"),bg='#d68709').place(x=25,y=y)
...
def lookup_gui(player_name, root):
    ...
    for entry in death_info_specific:
        print(entry, death_info_specific[entry])
    ...
    lbl_playername = tk.Label(root,text=player_name,font=("Helvetica", 16, "bold"),fg='#2bcaff', bg='#646970')
...
def create_gui():
    ...
    btn_player_data = tk.Button(root, text='Player Lookup', height=6, width=15, bd=5, font=("Helvetica", 10),
                                command=lambda: [lookup_gui(entry_username.get(), root)])
